...
package
{
	 ...
	 */
	enum bool isConvertibleToGDN(T) = isGDN!T || isImplicitlyConvertible!(T, real);
	 ...
	 */
	pure nothrow @nogc @safe real asReal(F)(in F f) if (isConvertibleToGDN!F)
	do {
		static if (isGDN!F)
			return f.val;
		else
	 ...
	 */
	template CommonGDN(G...) if (anySatisfy!(isGDN, G) && allSatisfy!(isConvertibleToGDN, G))
	{
		alias CommonGDN = GDN!(minDeg!G);
	}


	/* Exposes the GDN.dirac method as a function to help with overload
	 * resolution.
	 */
