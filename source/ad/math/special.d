...
do {
	...
	if (s == .0L) {
		...
		return x.val is real.infinity
			? GDN!Deg.one.d
			: x_red_pos^^(s - 1.0L) / (e(x_red_pos) * std.mathspecial.gamma(s));
	}
}
unittest {
	import std.format: format;
...
do {
	alias Q_inv = Select!(
		Deg == 1, std.mathspecial.gammaIncompleteComplInverse, gammaIncompleteComplInverse);
